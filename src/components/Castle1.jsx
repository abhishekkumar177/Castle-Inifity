...
const Castle1 = () => {
  ...
  useEffect(() => {
    ...
    const roofColors = [0x8B4513, 0x4A2C2A, 0x556B2F, 0x654321];
    ...
    const createSingleStructure = (numStories) => {
      ...
      const roofMaterial = new THREE.MeshBasicMaterial({ color: roofColors[Math.floor(Math.random() * roofColors.length)] });
      ...
      // Create the new simplified roof
      const roofPoints = [];
      const roofY = 1 + numStories * levelSpacing + 1.5;

      // Bottom plate
      const roofBottom = new THREE.Mesh(new THREE.BoxGeometry(30, 0.5, 30), roofMaterial);
      roofBottom.position.y = roofY;
      group.add(roofBottom);

      // Conical top
      const roofTop = new THREE.Mesh(new THREE.ConeGeometry(15, 10, 4), roofMaterial);
      roofTop.position.y = roofY + 5.25;
      group.add(roofTop);
